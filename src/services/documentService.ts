...
export async function uploadPublicPdf(file: File, input:{title:string;summary?:string;kind:'investment_proposal'|'pitch_deck'|'investor_report'|'business_update'|'supporting'}) {
  ...
  try {
    const asset=await supabase.from('media_assets').insert({bucket:'company-documents',path,original_filename:file.name,mime_type:'application/pdf',byte_size:file.size,visibility:'restricted',uploaded_by:user.user.id,finalized_at:new Date().toISOString()}).select('id').single();
...
export type AdminDocument = {
  id:string; title:string; summary:string|null; kind:string; status:'draft'|'review'|'approved'|'published'|'archived';
  visibility:string; updatedAt:string; versionNumber:number|null; fileName:string|null; mimeType:string|null; uploadedBy:string|null;
};

export async function listAdminDocuments():Promise<AdminDocument[]> {
  if(!supabase) return [];
  const {data,error}=await supabase.from('documents').select(
    'id,title,summary,kind,status,visibility,updated_at,current_version:document_versions!documents_current_version_id_fkey(version_number,file_asset:media_assets!document_versions_file_asset_id_fkey(original_filename,mime_type,uploaded_by))'
  ).eq('visibility','public').order('updated_at',{ascending:false});
  if(error) throw error;
  return (data||[]).map((d:any)=>({
    id:d.id,title:d.title,summary:d.summary,kind:d.kind,status:d.status,visibility:d.visibility,updatedAt:d.updated_at,
    versionNumber:d.current_version?.version_number??null,fileName:d.current_version?.file_asset?.original_filename??null,
    mimeType:d.current_version?.file_asset?.mime_type??null,uploadedBy:d.current_version?.file_asset?.uploaded_by??null,
  }));
}

export async function transitionAdminDocument(id:string,target:'draft'|'review'|'approved'|'published'|'archived') {
  if(!appSchema) throw new Error('Supabase production belum terhubung.');
  const {data,error}=await appSchema.rpc('transition_document_publication',{p_document_id:id,p_target:target});
  if(error) throw new Error(error.message);
  return data;
}

export async function getPublicDocumentDownload(id:string):Promise<{url:string;file_name:string}> {
  if(!supabase) throw new Error('Supabase production belum terhubung.');
  const {data,error}=await supabase.functions.invoke('public-document-download',{body:{document_id:id}});
  if(error || !data?.url) throw new Error(data?.error || error?.message || 'Dokumen tidak dapat diunduh.');
  return data;
}
